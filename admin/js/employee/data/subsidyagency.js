app.controller("SubsidyAgencyController", ['$scope', '$rootScope','$location','$http','$mdDialog','$window','subsidyEditService',function($scope, $rootScope,$location,$http,$mdDialog,$window,subsidyEditService) {
  ...
  var requesttabledata = function(){
    ...
    $http({
        method: 'post',
        url: http_url + "api/admin/staff/agency_policy_option",
        data:data,
        headers: {'Content-Type': 'application/json'}
        }).then(function successCallback(response) {
  ...
  $scope.submitSearch = function(ev) {
    ...
    $scope.clicksubsidytoedit = function(id) {
      subsidyEditService.addSubsidy(id,'/employee/data/subsidyagency');
